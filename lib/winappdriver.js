...
class WinAppDriver extends events.EventEmitter {
  ...
  async start (caps, emitStartingState = true) {
    ...
    const startDetector = (stdout) => {
        // XXXYD What is this for, looking for a log entry that shows we're up?
        // return stdout.indexOf('ENTER') != -1;
        return true; 
      };
    ...
    let processIsAlive = false;
     ...
     // handle out-of-bound exit by simply emitting a stopped state
      ...
      // XXXYD TODO: we need to handle status in WinAppDriver correctly in order to bring this wait back
  ...
  async waitForOnline () {
    // we need to make sure that WAD hasn't crashed
 ...
 async getStatus () {
    // XXXYD TODO: WinAppDriver isn't responding correctly to this
